...
class Mysql
	...
	private
	function showColumns($tableArray)
		{
		$db = dbConn::getConnection();
		$dbname = dbname;
		foreach($tableArray as $table => $tableName)
			{ //foreach for every table name fetch table name
			...
			while ($columnQuery = $query->fetch(PDO::FETCH_ASSOC))
				{
				foreach($columnQuery as $column => $columnname)
					{ //foreach for every table name fetch column name
	...
	private
	function showData($tableName, $columnArray)
		{
		$dbname = dbname;
